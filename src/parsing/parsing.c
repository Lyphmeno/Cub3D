...
int	parse_check_infull(t_data *data)
{
	if (!data->txr[0])
		return (-1);
	if (!data->txr[1])
...
int	parse_fill_info(t_data *data, char **arr)
{
	...
	else if (!ft_strncmp(arr[0], "C", ft_strlen(arr[0]))) // Need to take care of colors !
		data->sky = 1;
...
int	parse_info(t_data *data) // Parsing of the text above the map
{
	...
	while (tmp_line != NULL && parse_check_infull(data))
	{	
